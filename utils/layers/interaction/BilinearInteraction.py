...
class BilinearInteraction(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if self.bilinear_type == "all":
            ...
            p = [torch.multiply(vidots[i], inputs[j]) for i, j in combinations(range(n), 2)]
        ...
        # 特征交互 f_i.dot(w_ij) * f_j
            ...
            p = [torch.multiply(torch.matmul(v[0], w), v[1]) for v, w in zip(combinations(inputs, 2), self.W_list)]
        else:
            raise NotImplementedError
...
if __name__ == '__main__':
    ...
    bil = BilinearInteraction(input_shape=[v1.shape, v2.shape], bilinear_type="interaction")
    print(bil([v1, v2]))
