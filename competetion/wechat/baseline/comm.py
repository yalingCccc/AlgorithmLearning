...
# 存储数据的根目录
ROOT_PATH = "/data/wechat_algo_data1"

# 比赛数据集路径
DATASET_PATH = os.path.join(ROOT_PATH, "data_processed")
...
ACTION_SAMPLE_RATE = {"read_comment": 0.2, "like": 0.2, "click_avatar": 0.2, "forward": 0.1, "comment": 0.1,
...
def statis_feature(start_day=1, before_day=7, agg='sum'):
    ...
    for dim in ["userid", "feedid"]:
        ...
        feature_path = os.path.join(feature_dir, dim + "_feature_%s.csv" % before_day)
...
def concat_sample(sample_arr, stage="offline_train"):
    ...
    for index, sample in enumerate(sample_arr):
        ...
        for before_day in STATIS_DAYS:
            # 基于userid统计的历史行为的次数
            user_date_feature_path = os.path.join(ROOT_PATH, "feature", "userid_feature_%s.csv" % before_day)
            user_date_feature = pd.read_csv(user_date_feature_path)
            user_date_feature = user_date_feature.set_index(["userid", "date_"])
            user_date_feature.columns = [col + str(before_day) for col in user_date_feature.columns]

            # 基于feedid统计的历史行为的次数
            feed_date_feature_path = os.path.join(ROOT_PATH, "feature", "feedid_feature_%s.csv" % before_day)
            feed_date_feature = pd.read_csv(feed_date_feature_path)
            feed_date_feature = feed_date_feature.set_index(["feedid", "date_"])
            feed_date_feature.columns = [col + str(before_day) for col in feed_date_feature.columns]
            ...
            feed_feature_col = [b + 'sum' + str(before_day) for b in FEA_COLUMN_LIST]
            user_feature_col = [b + 'sum' + str(before_day) + "_user"for b in FEA_COLUMN_LIST]
...
def main():
    ...
    for days in STATIS_DAYS:
        statis_feature(1, days, 'sum')  # 统计物品、用户前一周行为次数
