...
# flags.DEFINE_string(变量名， 变量值， 变量描述)
...
class WideAndDeep(object):
    ...
    def __init__(self, linear_feature_columns, dnn_feature_columns, stage, action):
        """
        :param linear_feature_columns: List of ncf feature_column
        :param dnn_feature_columns: List of ncf feature_column
...
def get_feature_columns():
    ...
    # DNN features
    # fc.categorical_column_with_hash_bucket(key, hash_bucket_size, dtype)
    # 通常hash_bucket_size 设置为总类别数的2-5倍
    # 适用于总类别数有限的字段、适用于不能确定所有类别样式的类别变量
    user_cate = fc.categorical_column_with_hash_bucket("userid", 40000, tf.int64)  # user_id 20000
    feed_cate = fc.categorical_column_with_hash_bucket("feedid", 240000, tf.int64)  # feedid 96564
    ...
    # 把categorical的data，借由lookup table的方式找寻对应的feature vector来表示
    ...
    # 行为统计特征
    for before_day in [1, 3, 7]:
        for b in FEA_COLUMN_LIST:
            feed_b = fc.numeric_column(b + "sum" + str(before_day), default_value=0.0)
            linear_feature_columns.append(feed_b)
            user_b = fc.numeric_column(b + 'sum' + str(before_day) + '_user', default_value=0.0)
